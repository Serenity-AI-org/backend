...
def run_speech_tov_on_video(file_id, patient_id, session_id):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with srr.AudioFile(path) as source:
                while True:
                    try:
                        audio = r.record(source, duration=30)  # 30 seconds
                        text = r.recognize_google(audio, language="ar-AR")
                        print(text)
                    except srr.WaitTimeoutError:
                        break
                    except srr.UnknownValueError:
                        print("Could not understand chunk")
                    except srr.RequestError as e:
                        print(f"API error: {e}")
                        break
